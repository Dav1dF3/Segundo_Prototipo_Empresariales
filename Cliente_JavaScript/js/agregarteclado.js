...
async function agregarTeclado() {
  ...
  // ✅ VALIDAR CAMPOS OBLIGATORIOS (excepto marca)
  const codigo = document.getElementById("input-code-keyboard").value.trim();
  const nombre = document.getElementById("input-name-keyboard").value.trim();
  const precio = document.getElementById("input-price-keyboard").value.trim();
  const stock = document.getElementById("input-stock-keyboard").value.trim();
  const fecha = document.getElementById("input-date-keyboard").value.trim();
  const numeroTeclas = document.getElementById("input-keys-keyboard").value.trim();
  const sensibilidad = document.getElementById("select-sensitivity-keyboard").value.trim();
  const digitalChecked = document.getElementById("input-digital").checked;
  const analogChecked = document.getElementById("input-analog").checked;
  ...
  try {
    ...
    alert("✅ Teclado agregado correctamente");
    limpiarCampos();
...
function limpiarCampos() {
  // Campos del teclado
  document.getElementById("input-code-keyboard").value = "";
  ...
  document.getElementById("input-keys-keyboard").value = "";
  document.getElementById("select-sensitivity-keyboard").selectedIndex = 0;

  // Resetear radios de tipo (digital / analógico)
  document.getElementById("input-digital").checked = false;
  document.getElementById("input-analog").checked = false;
}
